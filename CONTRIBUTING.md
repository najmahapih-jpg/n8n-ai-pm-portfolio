...
Run live gates only when the required local n8n instance, Ollama, and credentials are configured:

```powershell
npm run verify:llm       # opt-in: real LLM planner via Ollama (llama3.2:3b)
npm run verify:tools     # opt-in: real gateway tool execution (needs GATEWAY_SIGNING_SECRET)
```
...
- Run the static, JSON, and core self-test gates.
