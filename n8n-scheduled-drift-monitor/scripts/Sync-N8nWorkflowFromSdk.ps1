param(
  ...
  [string]$SdkPath = ".\workflows\sdk\scheduled-drift-monitor.workflow.js",
  [string]$GeneratedPath = ".\workflows\generated\scheduled-drift-monitor.json",
  [string]$MetaPath = ".\workflows\sdk\scheduled-drift-monitor.meta.json",
  [int]$MinimumNodes = 18,
  [switch]$SkipCompile,
  [switch]$NoActivate
)
...
# 2a. D needs NO deploy-time secret wiring: its live HTTP calls (source-fetch, A-eval, Ollama digest)
#     read their URLs from the run config (host.docker.internal) inside onError-tolerant Code nodes —
#     there is no credential binding or secret placeholder in the tracked workflow JSON. (Live writes to
#     B's Supabase happen only in the reviewed host-side Refresh-Sources.ps1, never at deploy time.)
