...
Run the offline gates before opening a pull request (no running n8n needed):
...
Run live gates only when a running, imported, ACTIVE local n8n instance and credentials are configured. `npm run smoke` posts a stub PAYLOAD but still requires the live webhook, so it belongs here:
...
- Run the static and JSON gates (and the smoke + live gates when a local n8n is running).
