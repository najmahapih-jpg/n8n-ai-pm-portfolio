...
Run the live gate only when a local n8n instance with `GATEWAY_SIGNING_SECRET` and `NODE_FUNCTION_ALLOW_BUILTIN=crypto` configured is available:
...
- Review `docs/security-boundaries.md` when any interface, credential, security function, external adapter, or generated artifact changes.
- Run the static, JSON, gateway, and workflow gates.
- Redact credentials, webhook URLs, and any customer-like payloads from snapshots and fixtures.
