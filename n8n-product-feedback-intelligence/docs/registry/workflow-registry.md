...
| --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| Portfolio - Product Feedback Intelligence API | 0.2.0 | draft-tested | manual + webhook + executeWorkflowTrigger (callable sub-workflow) | 29 | advanced | n8n Manual Trigger, n8n Webhook, n8n Execute Workflow Trigger (passthrough) -- callable IN-PROCESS by the interaction-gateway (YKT4FJmC8Xg2G9hs); feeds the SAME Normalize Feedback Payload pipeline, webhook contract unchanged, Deterministic stub classifier (default), Live Ollama classifier (llama3.2:3b, per-request), Redacted audit | `workflows\releases\product-feedback-intelligence-v0.2.0.json` | `feedback-bug-negative` |
