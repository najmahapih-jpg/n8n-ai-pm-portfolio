...
5. Show the local draft workflow in n8n and call out the 21-node structure.
6. Run `scripts/Test-SupportTriageWorkflow.ps1` for all 8 official MCP pin-data paths.
7. Use `scripts/Export-N8nWorkflows.ps1` only as a broad Docker backup export.
8. Show the Git diff containing only safe docs, fixtures, scripts, SDK source, canonical workflow JSON, and release snapshots.
...
- Official MCP enhanced workflow validation: pass, 21 nodes
...
- Release snapshot: `workflows/releases/support-triage-v0.2.0.json`
- Draft export guardrail: API export plus `-MinimumNodes 21` validation
