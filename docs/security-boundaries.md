...
| Caller <-> n8n webhook | Workflows expose webhook APIs and test drivers POST JSON payloads. | Schema/required-field/size validation is DELEGATED to the deployment gateway — the workflow itself only coerces and trims inputs (it does not enforce max string/array sizes). Caller-supplied URL overrides are ignored on the unauthenticated webhook entrypoint (SSRF guard). Production exposure needs authentication, rate limits, TLS, request size limits, replay controls, and logging policy outside the workflow. |
