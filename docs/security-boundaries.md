...
| Caller <-> n8n webhook | Workflows expose webhook APIs and test drivers POST JSON payloads. | Validate schema, required fields, max string/array sizes, and mode switches. Production exposure needs authentication, rate limits, TLS, request size limits, replay controls, and logging policy outside the workflow. |
