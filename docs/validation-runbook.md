...
Expected result: official MCP validates the SDK source, updates the draft, API export captures the current draft, scrubbed canonical and release files match, both satisfy the 21-node floor, and all 8 pin-data regression cases pass.
...
pwsh -NoProfile -File .\scripts\Test-N8nWorkflowJson.ps1 -Path .\workflows\canonical\portfolio-support-triage-api.canonical.json -MinimumNodes 21
