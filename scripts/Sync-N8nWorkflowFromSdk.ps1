param(
  [string]$WorkflowId = "",
  [string]$SdkPath = ".\workflows\sdk\product-feedback-intelligence.workflow.js",
  [string]$MetaPath = ".\workflows\sdk\product-feedback-intelligence.meta.json",
  [string]$ArtifactName = "product-feedback-intelligence",
  [string]$GeneratedOutputDirectory = ".\workflows\generated",
  [string]$CanonicalOutputDirectory = ".\workflows\canonical",
  [string]$ReleasePath = ".\workflows\releases\product-feedback-intelligence-v0.2.0.json",
  [int]$MinimumNodes = 28,
  [string]$Description = "Local product feedback intelligence: validate intake, classify theme/sentiment/urgency with an LLM-in-the-loop stub + deterministic fallback, derive urgency, score priority, gate churn-risk/critical for human review, redact audit, respond.",
