...
# filesystem walk that skips the same paths .gitignore excludes. The eval-harness project is
# initialised as a Git repo only after the toolchain lands, so the offline gate must not depend
# on `git`.
$gitRoot = $null
try {
